...
uniform_initializer = jax.nn.initializers.VarianceScaling(
...
def get_critic_network(config, action_spec):
  ...
  if config.use_pixels:
    ...
    layers.append(hk.nets.MLP([*config.layer_size_network, out_dim]))
    ...
    if config.use_residual:
      layers.extend(
        [
          ...
          jax.nn.elu,
          hk.nets.MLP([out_dim], w_init=uniform_initializer,),
        ]
      )
    else:
  ...
  if config.decouple:
    layers.append(hk.Reshape(output_shape=(-1, config.num_bins)))

  return  hk.Sequential(layers)


class CriticDQN(hk.Module):
