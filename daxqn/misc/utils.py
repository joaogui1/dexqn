...
#     best_action = jnp.argmax(q_t_selector, action_shape, output_type=jnp.int32)
#     double_q_bootstrapped = indexing_ops.batched_index(q_t_value, best_action)
#     target = jax.lax.stop_gradient(r_t + pcont_t * double_q_bootstrapped)
...
#     if decouple:
#       td_error = jnp.mean(td_error, axis=[-1])

#     loss = 0.5 * (td_error ** 2.0)
#     return LossOutput(
...
#   with tf.name_scope("epsilon_greedy", values=[action_values, epsilon]):
  ...
  action_values = jnp.asarray(action_values)
  epsilon = jnp.asarray(epsilon, dtype=action_values.dtype)

  # We compute the action space dynamically.
  num_actions = jnp.asaaray(action_values.shape[-1], dtype=action_values.dtype)
  if decouple:
    num_actions *= jnp.asaaray(action_values.shape[-2], dtype=action_values.dtype)

  # Dithering action distribution.
  dither_probs = 1 / num_actions * jnp.ones_like(action_values)

  # Greedy action distribution, breaking ties uniformly at random.
  max_value = jnp.max(action_values, axis=-1, keepdims=True)
  greedy_probs = jnp.equal(action_values, max_value)
  greedy_probs = greedy_probs.astype(action_values.dtype)
  greedy_probs /= jnp.sum(greedy_probs, axis=-1, keepdims=True)
